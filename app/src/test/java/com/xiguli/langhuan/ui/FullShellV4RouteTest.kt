...
class FullShellV4RouteTest {
    ...
    fun rootUsesFirstClassShelfCreationReaderStoryAndEditorExperiences() {
        ...
        val story = File(root, "src/main/java/com/xiguli/langhuan/ui/story/StoryCleanExperience.kt").readText()
        ...
        // Reader is resume-first, measured-page based and supports both horizontal chapter boundaries.
        ...
        assertTrue(reader.contains("readerDisplayChapterTitleV13"))
        assertTrue(reader.contains("readerBodyWithoutDuplicateHeadingV13"))
        assertTrue(reader.contains("reflowAnchorOffset"))
        assertTrue(reader.contains("appliedLayoutKey"))

        // Player story surface stays visually clean while the mature runtime remains mounted invisibly.
        assertTrue(story.contains("说什么，或做什么"))
        assertTrue(story.contains("角色与世界管理"))
        assertTrue(story.contains("StoryManagementExperience("))
        assertTrue(story.contains("StoryPlayPanelV17("))
