...
class PromptAssembler(
    ...
    fun buildProse(request: GenerationRequest): PromptBundle {
        ...
        val knowledgeText = ChapterContractGuard.renderKnowledge(snapshot, chapterNumber)
        ...
        return PromptBundle(
            system = """
                ...
                写作硬规则：
                1. 【章节合同】是本章最高优先级。必须发生、绝不能发生、允许揭露、必须保密、人物进出状态和章末钩子都不得被文风、RAG、模型常识或临时发挥覆盖。
                2. 只完成【本章任务】。全书方向和本卷方向只是背景，不得把未来章节的谜底、转折、势力、能力、证据或终局提前搬进本章。
                3. 【信息边界账本】区分作者真相、人物认知和读者认知。人物只能基于已知信息行动；HIDDEN/HINT_ONLY 的秘密不得被正文直接确认。
                4. 锁定设定是事实，但“作者知道”不等于“读者现在应该知道”。除非本章合同明确授权，否则不要解释世界观，不要集中罗列规则。
                5. 把信息写成场景：人物行动、选择、对话、观察、记忆和后果。禁止把材料整理成报告、百科、案件清单、会议纪要或调查总结。
                6. 禁止正文中出现“他目前掌握的信息”“本章总结”“状态更新”“已确认事实”“伏笔”“章纲”“场景计划”“本章约X字”等创作后台措辞。
                7. 不要为了证明设定完整而一次性枚举大量姓名、职业、死法、规则或证据。若多个个案只承担同一叙事功能，选择最有戏剧价值的少数例子，其余用自然概括。
                ...
                14. 文风服从作品模板。避免AI腔：少用排比式解释、总结句、反复加粗式强调、同义复述和“不是A，是B”连续句型。
                15. 目标字数是节奏参考，不为凑字重复信息，也不要用清单填充篇幅。
                16. 只输出小说正文。不要 Markdown 标题、不要前言、不要后记、不要说明。
            """.trimIndent(),
            user = """
    ...
    fun buildQualityReview(request: GenerationRequest, prose: String): PromptBundle {
        ...
        return PromptBundle(
            system = """
                ...
                3. 人物知道了信息边界账本中明确 UNKNOWN 的事实，或把 HIDDEN/HINT_ONLY 的秘密直接确认给读者。
